import Link from "next/link";

import { localizeHref, type Locale } from "@/lib/i18n";

type NotFoundPageProps = {
  locale: Locale;
};

const COPY = {
  zh: {
    kicker: "Not Found",
    title: "这个页面不存在",
    description: "你可以回到首页继续走教程主线，或者直接进入资源聚合页。",
    home: "返回首页",
    resources: "浏览资源",
  },
  en: {
    kicker: "Not Found",
    title: "This page does not exist",
    description: "You can return to the homepage to stay on the learning path, or jump straight into the resource hub.",
    home: "Back to home",
    resources: "Browse resources",
  },
} as const;

export function NotFoundPage({ locale }: NotFoundPageProps) {
  const copy = COPY[locale];

  return (
    <div className="mx-auto flex min-h-[60vh] max-w-3xl flex-col items-center justify-center gap-6 px-4 text-center sm:px-6 lg:px-8">
      <p className="section-kicker">{copy.kicker}</p>
      <h1 className="font-[family-name:var(--font-serif)] text-4xl text-white">
        {copy.title}
      </h1>
      <p className="max-w-xl text-lg leading-8 text-white/68">{copy.description}</p>
      <div className="flex flex-wrap justify-center gap-3">
        <Link
          href={localizeHref(locale, "/")}
          className="rounded-full bg-[var(--color-accent)] px-5 py-3 text-sm font-medium text-white"
        >
          {copy.home}
        </Link>
        <Link
          href={localizeHref(locale, "/resources")}
          className="rounded-full border border-white/10 px-5 py-3 text-sm text-white/80"
        >
          {copy.resources}
        </Link>
      </div>
    </div>
  );
}
