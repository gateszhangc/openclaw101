...
description: "第一次上手别追求全部配完，先把 OpenClaw 真正跑起来，并在 Dashboard 里完成第一轮对话。"
episode: 2
roleName: "虾滑"
...
keyTakeaways:
  ...
  - "官方推荐主线是 curl 安装 CLI 后执行 openclaw onboard --install-daemon"
...
<Callout title="虾滑提醒" tone="tip">
  <p>“先在本机跑通，再迁移到云服务器”几乎总是最省时间的。</p>
  <p>你现在最需要的不是完美部署，而是第一次成功。</p>
</Callout>
...
<Callout title="虾滑的话" tone="warning">
  <p>第一次跑通时，最容易犯的错叫“配置过度”。</p>
  <p>你本来只需要一句“你好”，结果转头开始折腾远程访问、公开地址、群组机器人、自动化。</p>
  <p>这就是新手最常见的绕路方式。</p>
</Callout>
...
<Step number="01" title="把安装和首次启动命令真的跑一遍">
  <Command label="今天需要执行的最小命令集">
    {`curl -fsSL https://openclaw.ai/install.sh | bash
openclaw onboard --install-daemon
openclaw gateway status
openclaw status
...
<Step number="02" title="把第一次上手结果记下来">
  <ul>
    <li>哪一步一次成功</li>
    <li>哪一步最卡</li>
    <li>打开 Dashboard 以后你发出的第一句话是什么</li>
  </ul>
</Step>
