...
test("resources page supports direct entry filters plus search and filtering", async ({ page }) => {
  ...
  await browser.getByRole("button", { name: "清空筛选" }).click();
  await expect(browser.getByTestId("resource-card")).toHaveCount(10);
...
test("guide detail stays localized across locales", async ({ page }) => {
  await page.goto("/guide/03-models-and-auth", { waitUntil: "domcontentloaded" });
  await expect(
    page.getByRole("heading", {
      name: "OpenClaw 从零陪跑 03｜给它接上大脑：模型、Provider 和认证",
    }),
  ).toBeVisible();
  await expect(page.getByText("章节目录")).toBeVisible();
  ...
  await expect(
    page.getByRole("heading", {
      name: "OpenClaw from Scratch 03 | Hook Up the Brain: Models, Providers, and Auth",
    }),
  ).toBeVisible();
